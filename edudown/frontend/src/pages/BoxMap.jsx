...
export default function BoxMap({ user }) {
  ...
  const [urgenciaResult, setUrgenciaResult] = useState(null)

  const handleAgendarSuccess = (data) => {
    setAgendarBox(null)
    setUrgenciaModal(false)
    fetchBoxes(selectedSede)
    if (data?.desplazamiento) setUrgenciaResult(data)
  }
    ...
    <div className="space-y-5 max-w-5xl">
      ...
      {urgenciaResult && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-3 text-sm text-amber-800">
          ...
          <span className="flex-1">{urgenciaResult.mensaje}</span>
          <button onClick={() => setUrgenciaResult(null)} className="text-amber-400 hover:text-amber-600 transition-colors shrink-0">
